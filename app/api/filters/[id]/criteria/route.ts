...
function parseODataFilter(filterString: string) {
  ...
  console.log('Parsing filter:', filter);
    ...
    for (const fieldId of fieldIds) {
      try {
        const response = await fetch(
          `${PLOOMES_API_BASE}/Fields?$filter=Id eq ${fieldId}`,
...
export async function GET(
  ...
  try {
    ...
    // Buscar o filtro
    const response = await fetch(
      `${PLOOMES_API_BASE}/Filters?$filter=Id eq ${filterId}`,
      {
        headers,
        cache: "no-cache",
      }
    ...
    if (filter.Criteria) {
      ...
      console.log('Parsing OData criteria:', filter.Criteria);
