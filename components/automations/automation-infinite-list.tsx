...
const AutomationItem = ({ automation }: AutomationItemProps) => {
  const getStatusIcon = (status: string) => {
    switch (status) {
      case "active": return <CheckCircle2 className="h-3 w-3 text-green-600" />;
      case "inactive": return <XCircle className="h-3 w-3 text-zinc-500" />;
      case "error": return <AlertCircle className="h-3 w-3 text-red-600" />;
      default: return <Bot className="h-3 w-3 text-zinc-500" />;
    }
  };
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "inactive": return "text-zinc-600 bg-zinc-50 border-zinc-200";
  ...
  return (
    <Card className="hover:shadow-sm transition-shadow border-zinc-200 rounded-sm">
      <CardContent className="p-3">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-2 min-w-0 flex-1">
            <MapPin className="h-3 w-3 text-rose-500 mt-0.5 shrink-0" />
            <div className="min-w-0 flex-1">
              <h3 className="text-sm font-medium text-zinc-800 mb-1 line-clamp-2">
                {automation.name}
              </h3>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Badge
              variant="outline"
              className={cn("text-xs px-2 py-1 rounded-sm", getStatusColor(automation.status))}
            >
                ...
                <div className="space-y-4 text-sm">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-zinc-600 mt-1">{automation.entityName}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="text-zinc-600 mt-1">{automation.id}</p>
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-1 mt-2">
                        {automation.actions.map((action, index) => (
                          <div
                            key={action.id}
                            className="flex items-center justify-between text-xs py-2 px-2 bg-zinc-50 rounded-sm"
                          >
...
export const AutomationInfiniteList = ({
  entityId,
  status,
  search,
  createdBy,
  dateFrom,
  dateTo
}: AutomationInfiniteListProps) => {
  const observerRef = useRef<IntersectionObserver>();
  const lastAutomationElementRef = useRef<HTMLDivElement>();
    ...
    <div className="space-y-2 p-3">
      {allAutomations.map((automation, index) => {
        ...
        return (
          <div
            ...
            ref={isLast ? lastAutomationElementCallback : undefined}
          >
      ...
      {!hasNextPage && allAutomations.length > 0 && (
        <div className="text-center py-4 text-xs text-zinc-500">
          Todas as automações foram carregadas ({allAutomations.length} total)
        </div>
      )}
    </div>
  );
