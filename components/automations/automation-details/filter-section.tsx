...
export const FilterSection = ({ automation }: FilterSectionProps) => {
  ...
  const hasMultipleGroups = hasFilterCriteria &&
    new Set(automation.filterCriteria?.map(c => c.logicalGroup || 1)).size > 1;
                  ...
                  {automation.filterCriteria?.map((criterion, index) => (
                    <tr key={index} className="hover:bg-zinc-50 transition-colors">
                      <td className="px-3 py-2 text-zinc-700">
                        <Hint content={criterion.entity || 'Workflow'} side="top">
                          <div className="truncate max-w-[120px]">{criterion.entity || 'Workflow'}</div>
                        </Hint>
                      </td>
                      <td className="px-3 py-2 text-zinc-700 font-medium">
                        <Hint content={criterion.field} side="top">
                          <div className="truncate max-w-[180px]">{criterion.field}</div>
                        </Hint>
                      </td>
                      <td className="px-3 py-2 text-zinc-600">
                        <Hint content={criterion.operation} side="top">
                          <div className="truncate max-w-[120px]">{criterion.operation}</div>
                        </Hint>
                      </td>
                      <td className="px-3 py-2">
                        <Hint content={criterion.value} side="top">
                          <code className="text-xs bg-zinc-100 px-2 py-1 rounded border border-zinc-200 text-zinc-700 inline-block max-w-[200px] truncate align-middle">
                            {criterion.value}
                          </code>
                        </Hint>
                      </td>
